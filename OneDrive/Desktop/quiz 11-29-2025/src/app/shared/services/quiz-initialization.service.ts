...
export class QuizInitializationService {
  ...
  public async initializeQuiz(): Promise<void> {
    ...
    this.explanationTextService.initializeExplanationTexts(
      resolvedQuiz.questions.map((q: QuizQuestion) => q.explanation)
    );
  ...
  public subscribeToCurrentQuestion(): void {
    ...
    combinedQuestionObservable
      .pipe(
        filter(
          (question: QuizQuestion | null): question is QuizQuestion =>
            question !== null
        ),
        map((question) => ({
          ...question,
  ...
  private async getQuestion(): Promise<void | null> {
    try {
      ...
      const result = await firstValueFrom(
        of(
          this.quizDataService.fetchQuestionAndOptionsFromAPI(quizId, currentQuestionIndex)
        )
      );
  ...
  private fetchQuestionAndOptions(): void {
    ...
    this.quizDataService
      ...
      .subscribe(([question, options]) => {
        if (question && options) {
          // question is non-null inside this block, so cast safely
          this.quizStateService.updateCurrentQuizState(of(question as QuizQuestion));
        } else {
  ...
  private initializeObservables(): void {
    const quizId = this.activatedRoute.snapshot.paramMap.get('quizId');
    if (!quizId) {
      console.warn('No quizId found in route');
      return;
    }
  ...
  async fetchQuizData(): Promise<void> {
    try {
      ...
      if (!Array.isArray(selectedQuiz.questions)) return;
      if (
  ...
  private initializeQuizState(): void {
    ...
    this.quizService.findQuizByQuizId(this.quizId).subscribe({
      next: (currentQuiz) => {
        ...
        setTimeout(() => { this.updateQuizUIForNewQuestion(currentQuestion); }, 0);
      },
      error: (error) => {
