...
export class SharedOptionComponent implements OnInit, OnChanges {
  ...
  getOptionIcon(option: Option): string {
    // If feedback icons should not be shown, return no icon
    if (!this.showFeedback) return '';
  
    // Show 'close' (X mark) for incorrect options with feedback
    if (!option.correct && option.feedback === 'x') {
      return 'close'; // X mark for incorrect options with feedback
    }
  ...
  private generateFeedbackConfig(option: SelectedOption, index: number): FeedbackProps {
    return {
      ...
      correctMessage: option?.correct ? this.correctMessage ?? 'No correct message available' : '',
