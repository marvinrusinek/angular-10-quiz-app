  ...
  ngAfterViewInit(): void {
    setTimeout(() => {
      if (!this.optionBindings?.length && this.optionsToDisplay?.length) {
        console.warn('[⚠️ SOC] Forcing optionBindings generation AFTER viewReady delay');
        this.generateOptionBindings();
      }
    
      // Delay setting viewReady slightly more
      setTimeout(() => {
        this.viewReady = true;
        console.log('[✅ View ready]');
      }, 10); // buffer to let generateOptionBindings finish
    }, 50);    
  }
