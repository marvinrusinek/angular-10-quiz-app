...
export class QuizQuestionComponent extends BaseQuestionComponent 
  ...
  public override async onOptionClicked(event: {
    ...
    if (
      ...
      // FIRST incorrect click: block any flash for Next button / selection message
      this._firstClickIncorrectGuard.add(i0);
      this.selectionMessage = 'Select a correct option to continue...';
      this._skipNextAsyncUpdates = true; // prevent Next button / async updates
      // ✅ Still allow explanation and feedback to display
  }
  ...
  } else if (!isMulti && !evtOpt?.correct && this._firstClickIncorrectGuard.has(i0)) {
      // preserve first incorrect click message
