  ...
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  public override async onOptionClicked(event: { option: SelectedOption | null; index: number; checked: boolean; }): Promise<void> {
    try {
        ...
        // **🚀 Ensure Explanation Fetch for the Correct Question**
        console.log(`[onOptionClicked] 🔍 LOCKING explanation fetch to Q${this.currentQuestionIndex}`);
        const lockedQuestionIndex = this.currentQuestionIndex;

        // **Reset explanation before fetching to avoid stale data**
        ...
        // ✅ **Check if explanation for Q1 exists in state (prevents Q2's explanation from being used)**
        ...
        if (!explanationText) {
            console.log(`[onOptionClicked] 🚀 Fetching explanation text for Q${lockedQuestionIndex}...`);
            ...
            // **Store the explanation so it's not re-fetched incorrectly**
        ...
        // ✅ **Apply explanation text to UI (ONLY IF STILL ON THE SAME QUESTION)**
        ...
        // **Ensure correctness outcome is checked**
