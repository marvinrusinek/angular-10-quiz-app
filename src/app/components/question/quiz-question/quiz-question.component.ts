  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
      try {
        ...
        // Only set explanation text when isAnswered is true
        if (this.isAnswered) {
          this.showExplanationText();
        } else {
          this.showQuestionText();
  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
      try {
        ...
        // Display logic based on `isAnswered` state
        if (this.isAnswered) {
          console.log('Setting explanation text because question is answered.');
          this.showExplanationText();
        } else {
          console.log('Setting question text because question is not answered.');
          this.showQuestionText();
        }
      } catch (error) {
        console.error('Error parsing stored data or invalid data format:', error);
  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            // Ensure `showExplanationText()` is only called if `isAnswered` is true
            if (this.isAnswered) {
                console.log('Calling showExplanationText as question is marked answered.');
                this.showExplanationText();
            } else {
                console.log('Calling showQuestionText as question is not answered.');
                this.showQuestionText();
            }
        } catch (error) {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            if (Array.isArray(parsedOptions) && parsedOptions.every(option =>
                option && typeof option === 'object' && 'text' in option && 'correct' in option
            )) {
                this.optionsToDisplay = parsedOptions;
            } else {
                console.error('Invalid option structure:', parsedOptions);
                throw new Error('Invalid or null options format');
            }
