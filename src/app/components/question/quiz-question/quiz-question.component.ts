  ...
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  async ngOnInit(): Promise<void> {
    ...
    console.log(
      `[QuizQuestionComponent] 🟢 shouldDisplayOptions for Q${this.fixedQuestionIndex}:`,
      this.shouldDisplayOptions
    );

    console.log(
      `[QuizQuestionComponent] 🟢 Initial questionData for Q${this.fixedQuestionIndex}:`,
      this.questionData
    );
    console.log(
      `[QuizQuestionComponent] 🟢 Initial options for Q${this.fixedQuestionIndex}:`,
      this.options
    );

    console.log(
      `[QuizQuestionComponent] 🔄 Checking data in ngOnInit for Q${this.fixedQuestionIndex}:`,
      this.data
    );
    console.log(
      `[QuizQuestionComponent] 🔄 Checking shouldDisplayOptions in ngOnInit for Q${this.fixedQuestionIndex}:`,
      this.shouldDisplayOptions
    );

    console.log(
      `[QuizQuestionComponent] 🔄 Checking shouldDisplayOptions for Q${this.fixedQuestionIndex}:`,
      this.shouldDisplayOptions
    );

    console.log(
      `[QuizQuestionComponent] ✅ shouldDisplayOptions value:`,
      this.shouldDisplayOptions
    );
  ...
  private setOptionsToDisplay(): void {
    console.log(
      `[setOptionsToDisplay] Called for question:`,
      this.currentQuestion?.questionText
    );
    console.log(
      `[setOptionsToDisplay] Current question options before update:`,
  ...
  private async handleRouteChanges(): Promise<void> {
    this.activatedRoute.paramMap.subscribe(async (params) => {
      let questionIndex = +params.get('questionIndex');
      console.log(
        `🔄 [handleRouteChanges] Route param received: ${questionIndex}`
      );

      // ✅ Ensure we have loaded the questions before checking the index
      if (!this.questionsArray || this.questionsArray.length === 0) {
        console.warn(
          '[handleRouteChanges] Questions are not loaded yet. Fetching now...'
        );
        const loaded = await this.loadQuestion(); // Ensure this populates `questionsArray`
      ...
      if (
        ...
        console.warn(
          `⚠️ [handleRouteChanges] Invalid question index from route: ${questionIndex}. Defaulting to 0.`
        );
        questionIndex = 0;
      }
      ...
      this.currentQuestionIndex = questionIndex;
      console.log(
        `✅ [handleRouteChanges] Setting currentQuestionIndex: ${this.currentQuestionIndex}`
      );
      ...
      try {
        ...
        if (!this.currentQuestion) {
          console.error(
            '[handleRouteChanges] Current question is null or undefined after setting.'
          );
          return;
        }

        console.log(
          `✅ [handleRouteChanges] Loaded Question ${this.currentQuestionIndex}:`,
          this.currentQuestion.questionText
        );
        ...
        console.log(
          `✅ [handleRouteChanges] Options for Q${this.currentQuestionIndex}:`,
          this.optionsToDisplay
        );

        // ✅ Ensure `loadQuestion()` is triggered to fully update UI
        console.log(
          `🔵 [handleRouteChanges] Calling loadQuestion() for Q${this.currentQuestionIndex}`
        );
        await this.loadQuestion();
        ...
        if (isAnswered) {
          console.log(
            `✅ [handleRouteChanges] Question ${this.currentQuestionIndex} is already answered. Loading explanation...`
          );
  ...
  /* public override async onOptionClicked(event: { option: SelectedOption | null; index: number; checked: boolean; }): Promise<void> {
    try {
        ...
        // 🔒 **Ensure correct `lockedQuestionIndex` for explanation retrieval**
        const lockedQuestionIndex = this.currentQuestionIndex;
        ...
        // 🚀 **Fetch explanation from service if not already stored**
        if (!explanationText) {
            ...
            // ✅ Store explanation immediately under the correct index
            if (explanationText) {
  ...
  /* public override async onOptionClicked(event: { option: SelectedOption | null; index: number; checked: boolean; }): Promise<void> {
    try {
        ...
        // ✅ Prevent clicking before feedback is ready
        if (!this.isFeedbackApplied) {
            console.warn('[onOptionClicked] ⚠️ Feedback is not ready. Applying feedback...');
            await this.applyOptionFeedback(foundOption);
            console.log('[onOptionClicked] 🚀 Feedback applied.');
        }
        ...
        const lockedQuestionIndex = this.currentQuestionIndex; // ✅ Keep it fixed per question
        console.log(`[onOptionClicked] 🔒 LOCKED INDEX for Explanation Fetch: Q${lockedQuestionIndex}`);
  ...
  /* public override async onOptionClicked(event: { option: SelectedOption | null; index: number; checked: boolean; }): Promise<void> {
    try {
        ...
        // 🔒 **Step 1: Lock question index correctly**
        const lockedQuestionIndex = event.index; // ✅ Use event.index instead of currentQuestionIndex
        console.log(`[onOptionClicked] 🔒 LOCKED INDEX for Explanation Fetch: Q${lockedQuestionIndex}`);
  ...
  /* public override async onOptionClicked(event: { option: SelectedOption | null; index: number; checked: boolean; }): Promise<void> {
    try {
      ...
      // ✅ Diagnostic: Check optionsToDisplay status
      if (!this.optionsToDisplay || this.optionsToDisplay.length === 0) {
        console.warn(`[onOptionClicked] ⚠️ optionsToDisplay is empty for Q${lockedQuestionIndex}, populating...`);
        await new Promise(resolve => setTimeout(resolve, 50));
        this.optionsToDisplay = this.populateOptionsToDisplay();
        console.log('[onOptionClicked] ✅ optionsToDisplay populated:', this.optionsToDisplay);
      } else {
        console.log(`[onOptionClicked] ✅ optionsToDisplay already populated for Q${lockedQuestionIndex}:`, this.optionsToDisplay);
      }

      // ✅ Diagnostic: Verify foundOption
      const foundOption = this.optionsToDisplay.find(opt => opt.optionId === event.option?.optionId);
      if (!foundOption) {
        console.error(`[onOptionClicked] ❌ No matching option found for Q${lockedQuestionIndex}. optionId: ${event.option?.optionId}`);
        return;
      }
      console.log(`[onOptionClicked] ✅ Valid option found for Q${lockedQuestionIndex}:`, foundOption);

      if (!this.isFeedbackApplied) {
        console.warn(`[onOptionClicked] ⚠️ Feedback not applied yet for Q${lockedQuestionIndex}. Applying now...`);
        await this.applyOptionFeedback(foundOption);
        console.log(`[onOptionClicked] ✅ Feedback applied for Q${lockedQuestionIndex}.`);
      }

      if (!this.selectedOptionService.isAnsweredSubject.getValue()) {
        console.log(`[onOptionClicked] ✅ Marking Q${lockedQuestionIndex} as answered.`);
        this.selectedOptionService.isAnsweredSubject.next(true);
      }

      // ✅ Diagnostic: Explanation retrieval
      console.log(`[onOptionClicked] 🔍 Attempting to retrieve stored explanation for Q${lockedQuestionIndex}...`);
      let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);

      if (!explanationText) {
        console.log(`[onOptionClicked] ⚠️ No stored explanation for Q${lockedQuestionIndex}, fetching...`);
        explanationText = await firstValueFrom(
          this.explanationTextService.getFormattedExplanationTextForQuestion(lockedQuestionIndex)
        );
        console.log(`[onOptionClicked] 🚀 Fetched explanation for Q${lockedQuestionIndex}:`, explanationText);

        // Store immediately after fetching
        this.quizStateService.setQuestionExplanation(this.quizId, lockedQuestionIndex, explanationText);
        console.log(`[onOptionClicked] ✅ Stored fetched explanation for Q${lockedQuestionIndex}.`);
      } else {
        console.log(`[onOptionClicked] ✅ Using already stored explanation for Q${lockedQuestionIndex}:`, explanationText);
      }

      if (!explanationText?.trim()) {
        console.warn(`[onOptionClicked] ⚠️ Empty explanation fetched for Q${lockedQuestionIndex}, setting default.`);
        explanationText = 'No explanation available.';
      }

      // Apply explanation clearly to the UI
      console.log(`[onOptionClicked] ✅ Applying explanation text to UI for Q${lockedQuestionIndex}:`, explanationText);
      this.explanationToDisplay = explanationText;
      this.explanationToDisplayChange.emit(explanationText);
      this.showExplanationChange.emit(true);
      this.cdRef.detectChanges();

      // ✅ Additional diagnostic logs for correctness handling
      console.log(`[onOptionClicked] 🔍 Calling handleCorrectnessOutcome for Q${lockedQuestionIndex}...`);
      await this.handleCorrectnessOutcome(true);

      // Feedback display diagnostic
      console.log(`[onOptionClicked] ✅ Displaying feedback for optionId: ${event.option?.optionId}`);
      this.showFeedbackForOption[event.option?.optionId || 0] = true;
      this.cdRef.detectChanges();

      // Enable Next button diagnostic
      console.log(`[onOptionClicked] ✅ Enabling Next button for Q${lockedQuestionIndex}`);
      this.answerSelected.emit(true);

      setTimeout(() => {
        console.log(`[onOptionClicked] 🔄 Marking for check after Q${lockedQuestionIndex} option click.`);
        this.cdRef.markForCheck();
      });

      console.log(`[onOptionClicked] ✅ Complete for question ${lockedQuestionIndex}.`);
  ...
  private async fetchAndUpdateExplanationText(
    ...
    console.log(
      `[fetchAndUpdateExplanationText] 🚀 Called for Q${questionIndex}`
    );
    ...
    try {
      ...
      if (storedExplanation) {
        console.log(
          `[fetchAndUpdateExplanationText] ✅ Found explanation in session storage for Q${questionIndex}`
        );
      ...
      if (
        ...
        const cachedExplanation =
          this.explanationTextService.formattedExplanations[questionIndex]
            .explanation;
        console.log(
          `[fetchAndUpdateExplanationText] ✅ Found explanation in service cache for Q${questionIndex}`
        );
      ...
      console.log(
        `[fetchAndUpdateExplanationText] 🔄 Fetching explanation for Q${questionIndex}...`
      );
      ...
      console.log(
        `[fetchAndUpdateExplanationText] ✅ Explanation fetched for Q${questionIndex}:`,
        explanationText
      );
    ...
    if (!explanationText) {
      try {
        ...
        if (explanationText.trim() !== correctExplanation) {
          console.error(`[updateExplanationText] 🚨 Explanation mismatch for Question ${questionIndex}`);
          console.log(`[updateExplanationText] 🟢 Correct explanation:`, correctExplanation);
          console.log(`[updateExplanationText] 🔴 Fetched explanation:`, explanationText);
  
          explanationText = correctExplanation; // FORCE CORRECT explanation
        }
  
        this.quizStateService.setQuestionExplanation(this.quizId, questionIndex, explanationText);
      } catch (error) {
        console.error(`[updateExplanationText] ❌ Fetch Error for Question ${questionIndex}:`, error);
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    console.log(`🔵 [FORCE LOG] Entered updateExplanationText() with initial index: ${questionIndex}`);

    // 🔒 Always trust the currentQuestionIndex from component state
    const lockedQuestionIndex = this.currentQuestionIndex;
    console.log(`[updateExplanationText] 🔒 Locked question index is ${lockedQuestionIndex}`);

    if (!this.quiz || !this.quiz.questions || !this.quiz.questions[lockedQuestionIndex]) {
        console.error(`[updateExplanationText] ❌ ERROR: Question NOT FOUND at locked index ${lockedQuestionIndex}`);
        return;
    }

    console.log(`[updateExplanationText] ✅ Confirmed Question Exists for Q${lockedQuestionIndex}:`, 
        this.quiz.questions[lockedQuestionIndex]);
    ...
    console.log(`[updateExplanationText] ✅ Q${lockedQuestionIndex} is answered. Proceeding...`);

    // Try fetching stored explanation
    let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);
    if (explanationText) {
        console.log(`[updateExplanationText] ✅ Using stored explanation for Q${lockedQuestionIndex}:`, explanationText);
    } else {
        console.log(`[updateExplanationText] 🚀 No stored explanation found. Fetching for Q${lockedQuestionIndex}...`);
        try {
            explanationText = await firstValueFrom(
                this.explanationTextService.getFormattedExplanationTextForQuestion(lockedQuestionIndex)
            );
            ...
            // Immediately store after fetching
            this.quizStateService.setQuestionExplanation(this.quizId, lockedQuestionIndex, explanationText);
        } catch (error) {
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    const lockedQuestionIndex = questionIndex - 1;

    console.log(`[updateExplanationText] 🔒 Adjusted lockedQuestionIndex (zero-based): ${lockedQuestionIndex}`);
    console.log(`[updateExplanationText] 🔒 Using currentQuestionIndex: ${this.currentQuestionIndex}`);

    if (!this.quiz || !this.quiz.questions || this.quiz.questions.length === 0) {
        console.error(`[updateExplanationText] ❌ ERROR: Quiz questions NOT loaded!`);
        return;
    }

    if (lockedQuestionIndex < 0 || lockedQuestionIndex >= this.quiz.questions.length) {
        console.error(`[updateExplanationText] ❌ ERROR: Question NOT FOUND at adjusted index ${lockedQuestionIndex}`);
        return;
    }

    const question = this.quiz.questions[lockedQuestionIndex];
    console.log(`[updateExplanationText] ✅ Confirmed Question Exists for Q${lockedQuestionIndex}:`, question);

    // ✅ Check Question State (Ensure Answered)
    const questionState = this.quizStateService.getQuestionState(this.quizId, lockedQuestionIndex);
    ...
    if (!questionState || !questionState.isAnswered) {
        console.warn(`[updateExplanationText] ⚠️ Q${lockedQuestionIndex} has NOT been answered yet. Skipping explanation.`);
        return;
    }
    ...
    if (!explanationText) {
        console.log(`[updateExplanationText] 🚀 Fetching explanation for Q${lockedQuestionIndex}...`);
        try {
            explanationText = await firstValueFrom(
                this.explanationTextService.getFormattedExplanationTextForQuestion(lockedQuestionIndex)
            );
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    console.log(`[updateExplanationText] Current component index: ${this.currentQuestionIndex}`);

    // 🔒 Force the use of the correct index
    const lockedQuestionIndex = this.currentQuestionIndex;
    ...
    if (!this.quiz?.questions[lockedQuestionIndex]) {
        console.error(`[updateExplanationText] ❌ No question found at locked index ${lockedQuestionIndex}`);
        return;
    }

    const questionState = this.quizStateService.getQuestionState(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Checking answered state for Q${lockedQuestionIndex}:`, questionState);

    if (!questionState || !questionState.isAnswered) {
        console.warn(`[updateExplanationText] ⚠️ Question Q${lockedQuestionIndex} is unanswered. Skipping.`);
        return;
    }

    // ✅ Check for stored explanation first
    let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Stored explanation for Q${lockedQuestionIndex}:`, explanationText);
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    if (actualStoredExplanation && explanationText !== actualStoredExplanation) {
      console.error(`[updateExplanationText] ❌ Mismatch! Using correct stored explanation.`);
      explanationText = actualStoredExplanation;
    }
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    // ✅ Explicit correction: Subtract one only if the questionIndex is 1-based
    const lockedQuestionIndex = this.currentQuestionIndex > 0
    ...
    if (!this.quiz || !this.quiz.questions || !this.quiz.questions[lockedQuestionIndex]) {
      console.error(`[updateExplanationText] ❌ No question found at corrected index ${lockedQuestionIndex}`);
      return;
    }
    ...
    if (!questionState?.isAnswered) {
      console.warn(`[updateExplanationText] ⚠️ Q${lockedQuestionIndex} not answered yet. Skipping.`);
      return;
    }
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    console.log(`[updateExplanationText] 🔵 Called with questionIndex: ${questionIndex}`);
    console.log(`[updateExplanationText] 🔵 Component currentQuestionIndex: ${this.currentQuestionIndex}`);
  
    // ✅ Use the index passed in directly; don't adjust here
    const lockedQuestionIndex = questionIndex;
  
    console.log(`[updateExplanationText] 🔒 lockedQuestionIndex set to: ${lockedQuestionIndex}`);
  
    // ✅ Ensure question exists
    if (!this.quiz?.questions?.[lockedQuestionIndex]) {
      console.error(`[updateExplanationText] ❌ No question found at index ${lockedQuestionIndex}`);
      return;
    }
    ...
    if (!questionState?.isAnswered) {
      console.warn(`[updateExplanationText] ⚠️ Question ${lockedQuestionIndex} has NOT been answered yet.`);
      return;
    }
    ...
    if (!explanationText) {
      try {
        ...
        console.log(`[updateExplanationText] ✅ Explanation fetched from service for Q${lockedQuestionIndex}:`, explanationText);
  
        // ✅ Immediately store fetched explanation
        this.quizStateService.setQuestionExplanation(this.quizId, lockedQuestionIndex, explanationText);
      } catch (error) {
        console.error(`[updateExplanationText] ❌ Error fetching explanation for Q${lockedQuestionIndex}:`, error);
        explanationText = 'Error loading explanation.';
      ...
      console.log(`[updateExplanationText] ✅ Using already stored explanation for Q${lockedQuestionIndex}:`, explanationText);
    }
  
    if (!explanationText || explanationText.trim() === '') {
      console.warn(`[updateExplanationText] ⚠️ Empty explanation for Q${lockedQuestionIndex}, setting default message.`);
      explanationText = 'No explanation available.';
    }
    ...
    console.log(`[updateExplanationText] 🎯 Final explanation shown for Q${lockedQuestionIndex}:`, explanationText);
  } */
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    console.log(`🔵 [FORCE LOG] updateExplanationText called for questionIndex: ${questionIndex}`);
    ...
    if (actualStoredExplanation && explanationText !== actualStoredExplanation) {
        console.error(`[updateExplanationText] ❌ Mismatch Detected! Using stored explanation explicitly.`);
        explanationText = actualStoredExplanation;
    }
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    if (lockedQuestionIndex < 0) {
        console.error(`[updateExplanationText] ❌ ERROR: Invalid questionIndex ${lockedQuestionIndex}. Forcing to 0.`);
    ...
    console.log(`[updateExplanationText] ✅ Confirmed Question Exists for Q${lockedQuestionIndex}:`, this.quiz.questions[lockedQuestionIndex]);
    ...
    // ✅ **Ensure explanation retrieval for the correct question**
    let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Stored Explanation Q${lockedQuestionIndex}:`, explanationText);
    ...
    if (!explanationText) {
        try {
            ...
            // ✅ **Store explanation under correct index**
            this.quizStateService.setQuestionExplanation(this.quizId, lockedQuestionIndex, explanationText);
        } catch (error) {
            console.error(`[updateExplanationText] ❌ Error fetching explanation for Q${lockedQuestionIndex}:`, error);
            explanationText = 'Error loading explanation.';
    ...
    // ✅ **Ensure explanation is valid before applying**
    if (!explanationText || explanationText.trim() === '') {
        console.warn(`[updateExplanationText] ⚠️ Empty explanation for Q${lockedQuestionIndex}, setting default.`);
        explanationText = 'No explanation available.';
    }

    // 🚀 **Apply the Explanation to UI**
    console.log(`[updateExplanationText] 🟢 Applying explanation for Q${lockedQuestionIndex}:`, explanationText);
    this.explanationToDisplay = explanationText;
    this.explanationToDisplayChange.emit(explanationText);
    this.showExplanationChange.emit(true);
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    // ✅ **Fix Indexing for First Question**
    let lockedQuestionIndex = questionIndex;
    if (lockedQuestionIndex < 0) {
        console.error(`[updateExplanationText] ❌ ERROR: Invalid questionIndex ${lockedQuestionIndex}. Forcing to 0.`);
        lockedQuestionIndex = 0;
    ...
    // ✅ **Ensure the correct question state is used**
    const questionState = this.quizStateService.getQuestionState(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Question State for Q${lockedQuestionIndex}:`, questionState);
    ...
    // ✅ **Check stored explanation first**
    let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Stored Explanation for Q${lockedQuestionIndex}:`, explanationText);

    // 🚀 **Fetch explanation if missing**
    if (!explanationText) {
        try {
            explanationText = await firstValueFrom(
                this.explanationTextService.getFormattedExplanationTextForQuestion(lockedQuestionIndex)
    ...
    // 🔥 **Ensure explanation matches the correct question**
    const actualStoredExplanation = this.explanationTextService.formattedExplanations[lockedQuestionIndex]?.explanation;

    if (actualStoredExplanation && explanationText !== actualStoredExplanation) {
        console.error(`[updateExplanationText] ❌ Mismatch Detected! Overriding with correct explanation.`);
        explanationText = actualStoredExplanation;
    }
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    let lockedQuestionIndex = questionIndex < 0 ? 0 : questionIndex;
    console.log(`[updateExplanationText] 🔒 FINAL lockedQuestionIndex: ${lockedQuestionIndex}`);

    // 🚨 Ensure question exists at the index
    if (!this.quiz?.questions[lockedQuestionIndex]) {
        console.error(`[updateExplanationText] ❌ No question at index ${lockedQuestionIndex}`);
        return;
    }
    ...
    // ✅ Get stored explanation first
    let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Stored Explanation for Q${lockedQuestionIndex}:`, explanationText);

    // 🚀 **Fetch if explanation is missing**
    if (!explanationText) {
        try {
            explanationText = await firstValueFrom(
                this.explanationTextService.getFormattedExplanationTextForQuestion(lockedQuestionIndex)
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    // ✅ Explicitly use `this.currentQuestionIndex` for accuracy
    const lockedQuestionIndex = this.currentQuestionIndex;
    console.log(`[updateExplanationText] 🔒 Using lockedQuestionIndex: ${lockedQuestionIndex}`);
    ...
    // ✅ Ensure question is answered before fetching explanation
    const questionState = this.quizStateService.getQuestionState(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Question State for Q${lockedQuestionIndex}:`, questionState);
    ...
    // 🔥 Ensure explanation matches correct question
    const actualStoredExplanation = this.explanationTextService.formattedExplanations[lockedQuestionIndex]?.explanation;
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    // ✅ Ensure we are using the correct question index
    const lockedQuestionIndex = this.currentQuestionIndex;
    console.log(`[updateExplanationText] 🔒 Using lockedQuestionIndex: ${lockedQuestionIndex}`);
  ...
  /* async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    // ✅ Retrieve stored explanation if it exists
    let explanationText = this.quizStateService.getStoredExplanation(this.quizId, lockedQuestionIndex);
    console.log(`[updateExplanationText] 🔍 Stored Explanation for Q${lockedQuestionIndex}:`, explanationText);
    ...
    if (actualStoredExplanation && explanationText !== actualStoredExplanation) {
        console.error(`[updateExplanationText] ❌ Mismatch Detected! Using stored explanation.`);
        explanationText = actualStoredExplanation;
    }
  ...
  async updateExplanationText(questionIndex: number): Promise<void> {
    ...
    // ✅ Ensure the correct index is always used
    const lockedQuestionIndex = questionIndex; // Do NOT shift index here

    console.log(`[updateExplanationText] 🔄 Final Locked Index Before Fetching: Q${lockedQuestionIndex}`);

    if (!this.quiz || !this.quiz.questions || !this.quiz.questions[lockedQuestionIndex]) {
        console.error(`[updateExplanationText] ❌ No question data at locked index Q${lockedQuestionIndex}`);
        return;
    }
    ...
    // ✅ 🚀 Force fetching explanation if Q1 has incorrect data
    if (!explanationText || lockedQuestionIndex === 0) {
        console.log(`[updateExplanationText] 🚀 Fetching Explanation from Service for Q${lockedQuestionIndex}...`);
        try {
            explanationText = await firstValueFrom(
                this.explanationTextService.getFormattedExplanationTextForQuestion(lockedQuestionIndex)
            ...
            // ✅ Store the fetched explanation to ensure it doesn't get overwritten
            this.quizStateService.setQuestionExplanation(this.quizId, lockedQuestionIndex, explanationText);
        } catch (error) {
            console.error(`[updateExplanationText] ❌ ERROR fetching explanation for Q${lockedQuestionIndex}:`, error);
            explanationText = 'Error loading explanation.';
