  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            // Validate parsedQuestion structure
            ...
            // Validate parsedOptions structure
            if (Array.isArray(parsedOptions) && parsedOptions.length > 0 && parsedOptions.every(option => option && 'optionText' in option && 'correct' in option)) {
                this.optionsToDisplay = parsedOptions;
            } else {
                console.error('Parsed options are not valid or empty:', parsedOptions);
                throw new Error('Invalid or null options format');
        ...
        try {
            ...
            if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
                parsedOptions.forEach((option, index) => {
                    if (!option || typeof option !== 'object' || !('optionText' in option) || !('correct' in option)) {
  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            // Validate parsedQuestion structure
            if (parsedQuestion && typeof parsedQuestion === 'object' && 'questionText' in parsedQuestion && 'options' in parsedQuestion) {
                this.currentQuestion = parsedQuestion;
            } else {
                console.error('Parsed question structure is invalid or null:', parsedQuestion);
                throw new Error('Invalid or null question format');
  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            // Adjust the validation to match your actual option structure
            if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
                parsedOptions.forEach((option, index) => {
                    if (
                        !option ||
                        typeof option !== 'object' ||
                        !('text' in option) || // Match with 'text' property in your actual data
                        !('correct' in option) ||
                        !('optionId' in option)
                    ) {
                        console.error(`Invalid option structure at index ${index}:`, option);
  ...
  private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            // Enhanced validation for parsed options
            if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
                for (const [index, option] of parsedOptions.entries()) {
                    if (
                        !option ||
