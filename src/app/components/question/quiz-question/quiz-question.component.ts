  ...
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  public async applyOptionFeedbackToAllOptions(): Promise<void> { 
    try {
      ...
      // ✅ Delay feedback application to ensure data is ready
      setTimeout(() => {
        console.log(`[applyOptionFeedbackToAllOptions] Generating feedback for Q${this.currentQuestionIndex}`);
        ...
        if (!feedbackList || feedbackList.length === 0) {
          console.error('[applyOptionFeedbackToAllOptions] Feedback generation failed. Retrying in 50ms...');
          setTimeout(() => this.applyOptionFeedbackToAllOptions(), 50);
          return;
        }
        ...
        // ✅ Validate feedback list length
        if (feedbackList.length !== this.optionsToDisplay.length) {
          console.warn(`[applyOptionFeedbackToAllOptions] Feedback list length mismatch. Expected ${this.optionsToDisplay.length}, but got ${feedbackList.length}. Retrying...`);
          setTimeout(() => this.applyOptionFeedbackToAllOptions(), 50);
          return;
        }
        ...
        // ✅ Force UI update to prevent stale rendering
        this.cdRef.detectChanges();
