...
export class QuizQuestionComponent extends BaseQuestionComponent
  ...
  /* private restoreQuizState(): void {
    ...
    if (isNewSession) {
        console.info('No saved state found – starting with default question.');
        this.loadQuestion();  // Loads the initial question for a new session
        return;
    }
    ...
    if (storedIndex && storedQuestion && storedOptions && storedIsAnswered) {
        ...
        // Restore `isAnswered` and set `displayExplanation` only for the current question
        this.isAnswered = storedIsAnswered === 'true';
        this.displayExplanation = this.isAnswered && this.currentQuestionIndex === +storedIndex;

        console.log(`Restored currentQuestionIndex: ${this.currentQuestionIndex}, isAnswered: ${this.isAnswered}, displayExplanation: ${this.displayExplanation}`);

        // Set the display mode based on the restored state
        this.setDisplayMode(this.isAnswered);

        // Load the correct question display based on the restored state
        this.loadCurrentQuestion();
    } else {
        // Log an incomplete state warning only once per session
        if (!sessionStorage.getItem('incompleteStateLogged')) {
            console.warn('Stored state is incomplete, loading default question', {
                currentQuestionIndex: !!storedIndex,
                currentQuestion: !!storedQuestion,
  ...
  /* private restoreQuizState(): void {
    ...
    // Check for a new session without stored state
    if (!storedIndex && !storedIsAnswered) {
        console.info('No saved state found – starting with default question.');
        this.loadQuestion();
        return;
    ...
    // Restore the current question index and answered state if available
    this.currentQuestionIndex = storedIndex ? +storedIndex : this.currentQuestionIndex;
    this.isAnswered = storedIsAnswered === 'true';

    // Lock displayExplanation to match only the current question index
    this.displayExplanation = this.isAnswered;
  ...
  /* private restoreQuizState(): void {
    ...
    // Early return if no stored state is found
    if (!storedIndex && !storedIsAnswered) {
        console.info('No saved state found – starting with default question.');
        this.loadQuestion();
        return;
    ...
    // Restore current question index and answered state if they are available
    this.currentQuestionIndex = storedIndex ? +storedIndex : this.currentQuestionIndex;
    this.isAnswered = storedIsAnswered === 'true';
    this.displayExplanation = this.isAnswered;  // Set once and persist for current question
  ...
  /* private restoreQuizState(): void {
    ...
    // If no stored state, start from the beginning
    if (!storedIndex && !storedIsAnswered) {
    ...
    // Restore the question index and answered state directly
    this.currentQuestionIndex = storedIndex ? +storedIndex : this.currentQuestionIndex;
    this.isAnswered = storedIsAnswered === 'true';
    this.displayExplanation = this.isAnswered;  // Directly set for current question
