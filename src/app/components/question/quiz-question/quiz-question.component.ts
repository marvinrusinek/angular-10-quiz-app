  ...
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  /* private saveQuizState(): void {
    try {
      ...
      // Enhanced validation for currentQuestion
      if (this.isValidQuestion(this.currentQuestion)) {
        ...
        // Log warning only once if the question structure is invalid
        if (!sessionStorage.getItem('invalidQuestionLogged')) {
          console.warn('Invalid or incomplete current question. Removing stored question.', {
            currentQuestion: this.currentQuestion,
            hasQuestionText: this.currentQuestion ? 'questionText' in this.currentQuestion : false,
  ...
  /* private saveQuizState(): void {
    try {
        ...
        // Enhanced validation and logging only once if question is invalid
        if (
            this.currentQuestion &&
            typeof this.currentQuestion === 'object' &&
            ...
            sessionStorage.setItem('currentQuestion', JSON.stringify(this.currentQuestion));
            sessionStorage.removeItem('invalidQuestionLogged'); // Clear any prior warning flag
        } else if (!sessionStorage.getItem('invalidQuestionLogged')) {
            // Log only once if the question structure is invalid
            console.warn('Invalid or incomplete current question. Removing stored question.', {
                currentQuestion: this.currentQuestion,
                missingDetails: {
        ...
        // Enhanced validation for options to display
        if (
            Array.isArray(this.optionsToDisplay) &&
            this.optionsToDisplay.every(option =>
                option &&
  ...
  /* private saveQuizState(): void {
    try {
        ...
        // Validate and save current question if fully populated
        const isValidCurrentQuestion = (
            this.currentQuestion &&
            typeof this.currentQuestion === 'object' &&
            'questionText' in this.currentQuestion &&
            Array.isArray(this.currentQuestion.options) &&
        ...
        if (isValidCurrentQuestion) {
            sessionStorage.setItem('currentQuestion', JSON.stringify(this.currentQuestion));
            sessionStorage.removeItem('invalidQuestionLogged'); // Clear previous warning if valid
        } else {
            console.warn('Invalid or incomplete current question. Removing stored question.', {
                currentQuestion: this.currentQuestion,
                missingDetails: {
        ...
        // Save options only if validated
  ...
  /* private saveQuizState(): void {
    try {
        ...
        // Validate and save current question if it meets the necessary structure
        if (
            this.currentQuestion &&
            typeof this.currentQuestion === 'object' &&
            this.currentQuestion.questionText &&
            ...
            sessionStorage.setItem('currentQuestion', JSON.stringify(this.currentQuestion));
            sessionStorage.removeItem('invalidQuestionLogged'); // Clear any prior warnings
        } else {
            // Log only once to avoid repetitive warnings
            if (!sessionStorage.getItem('invalidQuestionLogged')) {
                console.warn('Invalid or incomplete current question. Removing stored question.', {
                    currentQuestion: this.currentQuestion,
                    missingDetails: {
                        hasQuestionText: !!this.currentQuestion?.questionText,
                        hasOptionsArray: Array.isArray(this.currentQuestion?.options),
                        optionsCount: Array.isArray(this.currentQuestion?.options) ? this.currentQuestion.options.length : 0
                    }
                });
  ...
  /* private saveQuizState(): void {
    try {
        ...
        if (isQuestionValid) {
            sessionStorage.setItem('currentQuestion', JSON.stringify(this.currentQuestion));
            sessionStorage.removeItem('invalidQuestionLogged'); // Clear previous warning if now valid
        } else {
            // Log details only once per session if question is invalid
            if (!sessionStorage.getItem('invalidQuestionLogged')) {
                console.warn('Invalid or incomplete current question. Removing stored question.', {
                    currentQuestion: this.currentQuestion,
                    missingDetails: {
                        hasQuestionText: !!(this.currentQuestion && 'questionText' in this.currentQuestion),
                        hasOptionsArray: Array.isArray(this.currentQuestion?.options),
                        optionsCount: Array.isArray(this.currentQuestion?.options) ? this.currentQuestion.options.length : 0
                    }
                });
                sessionStorage.setItem('invalidQuestionLogged', 'true'); // Prevent repeat warnings
            }
            sessionStorage.removeItem('currentQuestion');
        }
  ...
  /* private saveQuizState(): void {
    try {
        ...
        if (!this.currentQuestion) {
            console.warn('Skipping saveQuizState: currentQuestion is not initialized.');
            return;
        }
        ...
        if (isCurrentQuestionValid) {
            sessionStorage.setItem('currentQuestion', JSON.stringify(this.currentQuestion));
            sessionStorage.removeItem('invalidQuestionLogged'); // Clear warning flag
        } else {
            // Log once if question structure is invalid
            if (!sessionStorage.getItem('invalidQuestionLogged')) {
                console.warn('Invalid or incomplete current question. Removing stored question.', {
                    currentQuestion: this.currentQuestion,
                    missingDetails: {
  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        try {
            ...
            // Validate parsed question data before setting
            const parsedQuestion = JSON.parse(storedQuestion);
            const isValidParsedQuestion = (
                parsedQuestion &&
                typeof parsedQuestion === 'object' &&
                'questionText' in parsedQuestion &&
                Array.isArray(parsedQuestion.options) &&
                parsedQuestion.options.length > 0
            );

            if (isValidParsedQuestion) {
                this.currentQuestion = parsedQuestion;
            } else {
                console.warn('Invalid parsed question structure. Loading default question.');
                this.loadQuestion();
                return;
            }

            // Validate parsed options before setting
            const parsedOptions = JSON.parse(storedOptions);
            const isValidParsedOptions = (
            ...
            if (isValidParsedOptions) {
                this.optionsToDisplay = parsedOptions;
            } else {
                console.warn('Invalid parsed options structure. Loading default options.');
                this.loadQuestion();
                return;
            }
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null) {
        ...
        try {
            ...
            if (
                ...
                'questionText' in parsedQuestion &&
                Array.isArray(parsedQuestion.options) &&
                parsedQuestion.options.every(option => option && 'text' in option)
            ) {
                this.currentQuestion = parsedQuestion;
            } else {
                console.warn('Restoring incomplete or invalid question state. Loading default question.');
                this.loadQuestion();
                return;
            }
        } catch (error) {
        ...
        try {
            ...
            if (
                Array.isArray(parsedOptions) &&
                parsedOptions.every(option => option && typeof option === 'object' && 'text' in option)
            ) {
                this.optionsToDisplay = parsedOptions;
            } else {
                console.warn('Restoring incomplete or invalid options state. Loading default question.');
                this.loadQuestion();
                return;
            }
        } catch (error) {
  ...
  /* private restoreQuizState(): void {
    ...
    if (storedIndex !== null && storedQuestion !== null && storedOptions !== null && storedIsAnswered !== null) {
        this.currentQuestionIndex = +storedIndex;
        ...
        try {
            ...
            if (
                ...
                throw new Error('Incomplete or invalid question structure');
            }
        } catch (error) {
            console.warn('Failed to restore a valid question. Loading default question.');
            this.loadQuestion();
            return;
        }
        ...
        try {
            ...
            if (
                ...
                throw new Error('Incomplete or invalid options structure');
            }
        } catch (error) {
            console.warn('Failed to restore valid options. Loading default question.');
            this.loadQuestion();
            return;
        }
        ...
        // Only log this once per session if the stored state is incomplete
        if (!sessionStorage.getItem('incompleteStateLogged')) {
            console.warn('Stored state is incomplete, loading default question');
            sessionStorage.setItem('incompleteStateLogged', 'true');
        }
        this.loadQuestion();
    }
  } */
  /* private restoreQuizState(): void {
    ...
    const missingData = {
        currentQuestionIndex: storedIndex === null,
        currentQuestion: storedQuestion === null,
        optionsToDisplay: storedOptions === null,
        isAnswered: storedIsAnswered === null
    };

    // Check if any required data is missing
    if (missingData.currentQuestionIndex || missingData.currentQuestion || missingData.optionsToDisplay || missingData.isAnswered) {
        // Log details about the missing data once
        if (!sessionStorage.getItem('incompleteStateLogged')) {
            console.warn('Stored state is incomplete, loading default question', missingData);
            sessionStorage.setItem('incompleteStateLogged', 'true'); // Prevent duplicate logs
        }
        this.loadQuestion(); // Fallback to default question if any data is missing
        return;
    }

    // Clear the logging flag as the state is now complete
    sessionStorage.removeItem('incompleteStateLogged');

    // Parse and restore the stored data as previously
    this.currentQuestionIndex = +storedIndex;
    this.isAnswered = storedIsAnswered === 'true';
    
    // Parse and validate the current question
    try {
        const parsedQuestion = JSON.parse(storedQuestion);
    ...
    try {
        const parsedOptions = JSON.parse(storedOptions);
        if (Array.isArray(parsedOptions) && parsedOptions.every(option => option && typeof option === 'object' && 'text' in option)) {
            this.optionsToDisplay = parsedOptions;
        } else {
  ...
  private restoreQuizState(): void {
    ...
    if (storedIndex && storedQuestion && storedOptions && storedIsAnswered) {
        this.currentQuestionIndex = +storedIndex;
        
        // Parse and validate the current question
        try {
            const parsedQuestion = JSON.parse(storedQuestion);
            if (parsedQuestion && 'questionText' in parsedQuestion) {
                this.currentQuestion = parsedQuestion;
            } else {
        ...
        try {
            const parsedOptions = JSON.parse(storedOptions);
            if (Array.isArray(parsedOptions) && parsedOptions.every(option => option && 'text' in option)) {
                this.optionsToDisplay = parsedOptions;
            } else {
  ...
  public async fetchAndSetExplanationText(questionIndex: number): Promise<void> {
    ...
    try {
      ...
      if (!this.questionsArray || this.questionsArray.length === 0) {
        ...
        await this.loadQuizData(); // Ensure the data is loaded before continuing
      }
