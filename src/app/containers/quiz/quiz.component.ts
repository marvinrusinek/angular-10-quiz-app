...
export class QuizComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    this.ngZone.run(async () => {
        ...
        try {
            ...
            if (typeof restoredIndex !== 'number' || restoredIndex < 0 || restoredIndex >= totalQuestions) {
                ...
                restoredIndex = Math.min(this.currentQuestionIndex, totalQuestions - 1); // **Ensures valid index**
            }

            console.log('[restoreStateAfterFocus] ✅ Final question index for restoration:', restoredIndex);

            // 🔹 Prevent index reset on re-focus
            if (this.currentQuestionIndex !== restoredIndex) {
                this.currentQuestionIndex = restoredIndex;
            ...
            const badgeIndex = savedBadgeIndex ? JSON.parse(savedBadgeIndex) : restoredIndex + 1;
            this.quizService.updateBadgeText(badgeIndex, totalQuestions);
            console.log('[restoreStateAfterFocus] ✅ Updated badge text:', badgeIndex);

            // 🔹 Ensure URL updates correctly (prevent incorrect resets)
            const newUrl = `/quiz/${this.quizId}/${restoredIndex}`;
            if (this.router.url !== newUrl) {
                console.warn('[restoreStateAfterFocus] ⚠️ URL mismatch detected. Updating manually...');
                this.ngZone.run(() => this.router.navigate(['/quiz', this.quizId, restoredIndex], { replaceUrl: true }));
  ...
  /* async navigateToQuestion(questionIndex: number): Promise<void> {
    ...
    console.log(`[navigateToQuestion] 🔄 Updating currentQuestionIndex to: ${questionIndex}`);
    this.currentQuestionIndex = questionIndex;
    ...
    // Log new URL before navigating
    const newUrl = `/quiz/${this.quizId}/${questionIndex}`;
    console.log('[navigateToQuestion] 🔄 Navigating to URL:', newUrl);

    try {
      console.log('[navigateToQuestion] 🟢 Calling router.navigate()...');
      const navigationSuccess = await this.ngZone.run(() => 
        this.router.navigate(['/quiz', this.quizId, questionIndex], { replaceUrl: true })
      );
  ...
  /* async navigateToQuestion(questionIndex: number): Promise<void> {
    ...
    try {
      ...
      // Force router to update the URL even if the component is already active
        await this.ngZone.run(() => 
          this.router.navigate(['/quiz', this.quizId, questionIndex], { 
          replaceUrl: true, // forces router to replace history entry
          skipLocationChange: false // ensures URL updates in address bar
        })
      );
    ...
    try {
        ...
        await this.ngZone.run(async () => {
            const navigationSuccess = await this.router.navigate(['/quiz', this.quizId, questionIndex], {
                replaceUrl: false, // Keep this false to ensure new history entry
                skipLocationChange: false // Make sure it updates the address bar
            });
        ...
        // Final badge update after full navigation
        this.quizService.updateBadgeText(questionIndex + 1, this.totalQuestions);

        console.log('[navigateToQuestion] ✅ Updated currentQuestion:', this.currentQuestion);
        console.log('[navigateToQuestion] ✅ Updated optionsToDisplay:', this.optionsToDisplay);

        // ✅ Update `currentQuestionIndex` ONLY AFTER successful navigation
        this.currentQuestionIndex = questionIndex;
  ...
  /* async navigateToQuestion(questionIndex: number): Promise<void> {
    ...
    try {
        console.log('[navigateToQuestion] 🟢 Calling router.navigate()...');
        await this.ngZone.run(() => 
            this.router.navigate(['/quiz', this.quizId, questionIndex], { replaceUrl: false, skipLocationChange: false })
        );
