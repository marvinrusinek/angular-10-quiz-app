...
export class QuizComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  @ViewChild('nextButton', { static: true, read: MatTooltip }) 
  ...
  onOptionSelected(event: { option: SelectedOption; index: number; checked: boolean }, isUserAction: boolean = true): void {
    ...
    // Manually refresh the tooltip to reflect the updated state
    this.refreshTooltip();
  ...
  private initializeTooltip(): void {
    this.nextButtonTooltip$ = combineLatest([
      ...
      this.isButtonEnabled$.pipe(
        distinctUntilChanged(),
        tap((value) => console.log('isNextButtonEnabled$ emitted:', value))
      )
    ]).pipe(
      map(([isSelected, isEnabled]) => {
        const tooltipText = isSelected && isEnabled 
          ? 'Next Question >>' 
          : 'Please select an option to continue...';
