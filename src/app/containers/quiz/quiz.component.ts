...
import { 
  ChangeDetectionStrategy, ChangeDetectorRef, Component, HostListener, Input, NgZone, OnDestroy, OnInit 
} from '@angular/core';
import { FormControl, FormGroup } from '@angular/forms';
...
export class QuizComponent implements OnInit, OnDestroy {
  @Input() data: { questionText: string; correctAnswersText?: string; currentOptions: Option[] };
  @Input() shouldDisplayNumberOfCorrectAnswers = false;
  @Input() selectedQuiz: Quiz = {} as Quiz;
  @Input() form: FormGroup;
  formControl: FormControl;
  ...
  get shouldApplyLastQuestionClass(): boolean {
    return this.questionIndex === this.totalQuestions;
  }

  get shouldHidePrevQuestionNav(): boolean {
    return this.currentQuestionIndex === 0;
  }
  ...
  get shouldHideShowResultsButton(): boolean {
    return !this.isQuizDataLoaded || this.currentQuestionIndex < this.totalQuestions - 1;
  }

  get shouldHideRestartNav(): boolean {
    return this.currentQuestionIndex === 0 || this.currentQuestionIndex === this.selectedQuiz?.questions.length - 1;
  }
  ...
  async fetchQuizData(): Promise<void> {
    try {
      ...
      const selectedQuiz: Quiz = await firstValueFrom(this.quizDataService.getQuiz(quizId).pipe(takeUntil(this.destroy$)));
      if (!selectedQuiz) {
        console.error('Selected quiz not found for quizId:', quizId);
        return;
      }
  ...
  private fetchQuestionAndOptions(): void {
    ...
    this.quizDataService.getQuestionAndOptions(this.quizId, this.questionIndex).pipe(
      map(data => Array.isArray(data) ? data : [null, null]),
      map(([question, options]) => [question || null, options || null]),
      catchError(error => {
        console.error('Error fetching question and options:', error);
  ...
  getQuestionAndOptions(quizId: string, questionIndex: number): void {
    this.quizDataService.getQuestionAndOptions(quizId, questionIndex).subscribe({
      next: ([question, options]) => {
        this.question = question;
        this.options = options;
      },
  ...
  refreshQuestionOnReset(): void {
    this.quizService.getCurrentQuestion().pipe(takeUntil(this.unsubscribe$)).subscribe((question: QuizQuestion) => {
      this.currentQuestion = question;
      this.options = question?.options || [];
    });
  }
  ...
  private async fetchQuestionData(quizId: string, questionIndex: number): Promise<QuizQuestion> {
    try {
      const rawData = await firstValueFrom(
        this.quizService.getQuestionData(quizId, questionIndex)
      );
  ...
  private initializeAndPrepareQuestion(questionData: QuizQuestion, quizId: string): void {
    this.data = questionData;
    this.quizService.setQuizId(quizId);
    this.quizService.fetchQuizQuestions();
    this.quizService.setQuestionData(questionData);
  ...
  private async getQuestion(): Promise<void | null> {
    try {
      ...
      const result = await firstValueFrom(
        of(this.quizDataService.fetchQuestionAndOptionsFromAPI(quizId, currentQuestionIndex))
      );
  ...
  private async getNextQuestion(): Promise<void> {
    try {
      const nextQuestion = await this.quizService.getNextQuestion(this.currentQuestionIndex);
  ...
  private async navigateToQuestion(questionIndex: number): Promise<void> {
    this.explanationTextService.setShouldDisplayExplanation(false);
    this.explanationTextService.resetStateBetweenQuestions();
  ...
  private async prepareQuestionForDisplay(questionIndex: number): Promise<void> {
    await this.fetchAndSetQuestionData(questionIndex);
    this.initializeQuestionForDisplay(questionIndex);
    this.updateQuestionDisplay(questionIndex);
    this.updateExplanationText(questionIndex);
  ...
  private initializeQuestionForDisplay(questionIndex: number): void {
    if (!Array.isArray(this.questions) || questionIndex >= this.questions.length) {
      console.error(`Questions not loaded or invalid index: ${questionIndex}`);
      return;
    }
  ...
  private updateNavigationAndExplanationState(): void {
    this.quizService.currentQuestionIndexSource.next(this.currentQuestionIndex);
    this.updateExplanationText(this.currentQuestionIndex);
    this.initializeOrUpdateQuestionState(this.currentQuestionIndex);
    this.updateProgressPercentage();
  ...
  private setDisplayStateForExplanationsAfterRestart(): Promise<void> {
    return new Promise((resolve, reject) => {
      const explanation = this.explanationTextService.getFormattedExplanationTextForQuestion(this.currentQuestionIndex);
      if (explanation) {
        this.explanationTextService.setExplanationText(explanation);
