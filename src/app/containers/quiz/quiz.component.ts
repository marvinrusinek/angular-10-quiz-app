...
export class QuizComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  /* onOptionSelected(event: {
    ...
    if (event.checked) {
      if (
        !this.selectedOptions.some((o) => o.optionId === event.option.optionId)
      ) {
        this.selectedOptions.push(event.option);
      }
    } else {
  ...
  /* onOptionSelected(event: {option: SelectedOption, index: number, checked: boolean}): void {
    ...
    console.log('After update - selectedOptions:', this.selectedOptions);
    console.log('After update - isNextButtonEnabled:', this.isNextButtonEnabled);
    console.log('After update - currentQuestionAnswered:', this.currentQuestionAnswered);
  ...
  /* onOptionSelected(event: {option: SelectedOption, index: number, checked: boolean}): void {
    ...
    this.isNextButtonEnabled = this.selectedOptions.length > 0;
    this.currentQuestionAnswered = this.isNextButtonEnabled;
    this.isButtonEnabledSubject.next(this.isNextButtonEnabled);  // Make sure this line is present
  ...
  onOptionSelected(event: {option: SelectedOption, index: number, checked: boolean}): void {
    ...
    this.quizStateService.setLoading(false);  // Ensure loading is set to false
  ...
  async advanceToNextQuestion(): Promise<void> {
    ...
    console.log('Current state - isNavigating:', this.isNavigating);
    console.log('Current state - currentQuestionAnswered:', this.currentQuestionAnswered);
