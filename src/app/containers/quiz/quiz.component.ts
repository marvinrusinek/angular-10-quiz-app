...
import { catchError, debounceTime, distinctUntilChanged, filter, map, mapTo, retry, shareReplay, startWith, switchMap, take, takeUntil, tap } from 'rxjs/operators';
...
export class QuizComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
        ...
        switchMap((params: ParamMap) => {
          ...
          return this.handleRouteParams(params).pipe(
            switchMap(({ quizData }) => {
              ...
              // return this.quizService.getQuestionByIndex(adjustedIndex);
              return from(this.quizQuestionLoaderService.fetchAndSetQuestionData(adjustedIndex)).pipe(
                tap((success) => {
                  if (!success) {
                    console.error(`[Route Init] ❌ fetchAndSetQuestionData failed for Q${adjustedIndex}`);
                  } else {
                    console.log(`[Route Init] ✅ Fetched Q${adjustedIndex}`);
                  }
                }),
                mapTo(null)
              );              
  ...
  private async fetchAndSetQuestionData(questionIndex: number): Promise<boolean> {
    ...
    try {
      ...
      this.quizQuestionLoaderService.setQuestionDetails(trimmedText, finalOptions, explanationText);
      this.currentQuestionIndex = questionIndex;
