...
export class FeedbackService {
  ...
  setCorrectMessage(correctOptions: Option[], optionsToDisplay: Option[]): string {
    console.log('[setCorrectMessage] Debug - Correct options:', correctOptions);
    console.log('[setCorrectMessage] Debug - Options to display:', optionsToDisplay);
    ...
    const correctOptionIndices = correctOptions.map((correctOption) => {
      ...
      if (originalIndex === -1) {
        console.warn(`[setCorrectMessage] Correct option with ID "${correctOption.optionId}" not found.`);
      }
