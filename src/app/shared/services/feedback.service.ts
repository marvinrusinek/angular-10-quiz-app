...
export class FeedbackService {
  ...
  /* setCorrectMessage(correctOptions: Option[], optionsToDisplay: Option[]): string {
    ...
    const correctOptionIndices = correctOptions.map((correctOption) => {
      const normalizedCorrectText = correctOption.text.trim().toLowerCase();
      const originalIndex = optionsToDisplay.findIndex(
        (option) => option.text.trim().toLowerCase() === normalizedCorrectText
      );
  ...
  /* setCorrectMessage(correctOptions: Option[], optionsToDisplay: Option[]): string {
    ...
    const correctOptionIndices = correctOptions.map((correctOption) => {
      const originalIndex = optionsToDisplay.findIndex((option) => {
        // Match by optionId first, fallback to matching by trimmed text
        return (
          (option.optionId && correctOption.optionId && option.optionId === correctOption.optionId) ||
          (option.text.trim().toLowerCase() === correctOption.text.trim().toLowerCase())
        );
      });
  ...
  setCorrectMessage(correctOptions: Option[], optionsToDisplay: Option[]): string {
    ...
    const correctOptionIndices = correctOptions.map(correctOption => {
      const matchedIndex = optionsToDisplay.findIndex(option => {
        const isMatch = option.optionId === correctOption.optionId;
        if (!isMatch) {
          console.warn('[setCorrectMessage] Mismatch found:', {
            correctOption,
            optionInOptionsToDisplay: option,
          });
        }
        return isMatch;
      });
  
      return matchedIndex !== -1 ? matchedIndex + 1 : undefined; // Use 1-based index
    });
