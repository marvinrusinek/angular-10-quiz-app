...
export class SelectedOptionService {
  ...
  /* updateAnsweredState(): void {
    const selectedOptions = Array.from(this.selectedOptionsMap.values()).flat();
    console.log('Selected options for updateAnsweredState:', selectedOptions);
    ...
    if (this.currentQuestionType === QuestionType.MultipleAnswer) {
      ...
      console.log('Multiple-answer state:', { allCorrectSelected, hasAnySelected, isAnswered });
    } else {
      isAnswered = selectedOptions.some((option) => option.selected);
      console.log('Single-answer state:', { isAnswered });
    }

    this.setAnsweredState(isAnswered);
    console.log('Updated answered state:', { currentQuestionType: this.currentQuestionType, isAnswered });
  } */
  /* updateAnsweredState(): void {
    const selectedOptions = Array.from(this.selectedOptionsMap.values()).flat();
    console.log('Selected options for updateAnsweredState:', selectedOptions);
    ...
    if (this.currentQuestionType === QuestionType.MultipleAnswer) {
        ...
        console.log('Multiple-answer question isAnswered state:', { isAnswered });
    } else {
        // For single-answer, ensure at least one option is selected
        isAnswered = selectedOptions.some((option) => option.selected);
        console.log('Single-answer question isAnswered state:', { isAnswered });
    }
  ...
  /* updateAnsweredState(): void {
    ...
    if (this.currentQuestionType === QuestionType.MultipleAnswer) {
      // Ensure all required options are selected for multiple-answer questions
      isAnswered = selectedOptions.every((option) => option.correct ? option.selected : true);
    } else {
      // Single-answer logic: At least one option selected
      isAnswered = selectedOptions.some((option) => option.selected);
    }
  ...
  /* updateAnsweredState(): void {
    ...
    if (this.currentQuestionType === QuestionType.MultipleAnswer) {
      // Multiple-answer logic: Check if at least one option is selected
      isAnswered = selectedOptions.some((option) => option.selected);
  
      // Optionally, check if all correct options are selected
      const allCorrectSelected = selectedOptions.every(
        (option) => option.correct === option.selected
      );
      isAnswered = isAnswered && allCorrectSelected;
    } else {
      // Single-answer logic: Check if any option is selected
      isAnswered = selectedOptions.some((option) => option.selected);
    }
  ...
  updateAnsweredState(): void {
    ...
    if (this.currentQuestionType === QuestionType.MultipleAnswer) {
      // Multiple-answer logic
      isAnswered = selectedOptions.every(
      ...
      console.warn(
        'SelectedOptionService: currentQuestionType is not set or unknown. Assuming unanswered.'
      );
      isAnswered = false;
    }
