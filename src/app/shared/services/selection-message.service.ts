...
export class SelectionMessageService {
  ...
  /* public computeFinalMessage(args: {  
    ...
    // ───────── SINGLE-ANSWER ─────────
    if (qType === QuestionType.SingleAnswer) {
      // 🛡️ Sticky baseline: hold until user actually clicks
      if (
        selectedCorrect === 0 &&
        selectedWrong === 0 &&
        !this._singleAnswerCorrectLock.has(index) &&
    ...
    if (qType === QuestionType.MultipleAnswer) {
      ...
      // 🛡️ Sticky baseline: enforce until at least one correct is chosen
      if (selectedCorrect === 0 && !this._multiAnswerCompletionLock.has(index)) {
        this._multiAnswerPreLock.add(index);
        return baselineMsg;
      }
  ...
  public computeFinalMessage(args: {  
    ...
    if (qType === QuestionType.MultipleAnswer) {
      ...
      if (selectedCorrect === 0) {
        // still baseline until a correct is chosen
        return baselineMsg;
      }
