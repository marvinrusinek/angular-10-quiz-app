...
export class SelectionMessageService {
  ...
  public emitFromClick(params: {  
    ...
    if (questionType === QuestionType.MultipleAnswer) {
      ...
      if (index === 3) { // Handling Q4 dynamically
        const option1Selected = options.some((opt: any) => opt.text === 'Option 1' && opt.selected);
        const option2Selected = options.some((opt: any) => opt.text === 'Option 2' && opt.selected);
        ...
        if (option1Selected && option2Selected && !option3Selected) {
          message = "Select 1 more correct answer to continue..."; // For Q4, if Option 1 and Option 2 are selected but Option 3 is not
        } else if (option3Selected) {
          message = NEXT_MSG; // If Option 3 is selected, show "next" message
        }
      }
